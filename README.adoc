...
- https://github.com/ring-clojure/ring/wiki[Ring] handler consists of a simple function that receives a req (hash-map) and returns a response (also hash-map).
...
.Using reader macro to convert cljs to js:
[source, clojure]
----
(let [a #js [1 2 3]]
  (println (aget a 1)))
...
New tree structure of `src/` directory for ClojureScript sources.
...
- The `:require` statement on `ns` can load any Google Closure module or your defined module that the compiler can find on the path (see _project.clj_ for path...)
...
Little preview of routing in the browser. Some JavaScript frameworks come with their own solution, but in
...
today. For it, firstly define the following util functions:
...
Now having the background of ES7 example, let's see same thing but using
...
- put in a returned channel the result of last expression.
...
to make the logic look like sync, removing visually accidental complexity of continuations/callbacks.
...
Add the corresponding dependency of core.async to our project.clj:
...
Synchronous code makes it easier to reason about itself.
...
Before see a more complex app, we'll try understand the basics of Om components.
...
In previous examples we have used a few protocols. Om comes with a few other
...
- The state of the application is serializable, that makes it easy and in deterministic way to reproduce
...
- The union of ClojureScript and React makes some task, that is usually considered very complex,
  very easy and painless, such as time traveling or undo in a few lines of code.
...
Now, each change in our application, is saved as a snapshot in another atom, and
...
Now having experimented with time traveling, let's try an little experiment making the state persistent.
...
offers an abstraction for running a REPL in different evaluation environments.
...
Browser REPL is awesome if you are building a web app, but it is slightly tedious if
...
engine. You can start a Node.js REPL (as example) much easier than a browser REPL.
...
At this moment, we have compiled ClojureScript for development friendly environments. And
...
out of the box with the Closure compiler.

But it is not a problem. ClojureScript thanks to some Closure compiler options allow easy
...
Externs file is a file with special only definition JavaScript. They are very tedious to write
